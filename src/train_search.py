...
def main(primitives):
  ...
  if args.resume:
      if os.path.isfile(args.resume_file):
          ...
          architect_init.optimizer.load_state_dict(checkpoint['arch_optimizer'])
          scheduler_init = CosineAnnealingLR(optimizer_init, float(args.epochs), eta_min=args.learning_rate_min)
          analyser_init = Analyzer(args, model_init)
          la_tracker = utils.EVLocalAvg(args.window, args.report_freq_hessian,
                                  args.epochs)
  ...
  def train_epochs(epochs_to_train, iteration,
    ...
    for epoch in range(start_epoch+1, epochs_to_train):
      ...
      final_iteration = round(
          np.log(args.max_weight_decay) / np.log(args.weight_decay), 1) == 1. ##lr decay到一定程度就停止
